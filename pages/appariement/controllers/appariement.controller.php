...
if (isset($_POST['Envoyer'])) {
  ...
  if (strlen($historique_data['fichier']) > 1 && $_POST['etude']) {
      ...
      if (strtoupper($extension) === 'CSV') {
        ...
        $encodage = mb_detect_encoding(file_get_contents(PATH_DATA .$historique_data['fichier']));
...
if (isset($_POST['file_variables'])) {
  $nom_etude = isset($_POST['etude'])?$_POST['etude']:$nom_etude;
  $file = $_FILES['file_variables']['name'];
  $class_details_etude = 'tab-pane fade in active';
  $format_fichier_data = isset($_POST['format_fichier'])?$_POST['format_fichier']:"1";
  if (strlen($_FILES['file_variables']['name']) > 1) {
    $extension = pathinfo($_FILES['file_variables']['name'], PATHINFO_EXTENSION);
    if (strtoupper($extension) == 'XLSX' or strtoupper($extension) == 'XLS') {
      $b_extension = true;
      $file_name_destination = pathinfo($_FILES['file_variables']['name'], PATHINFO_FILENAME) . '_' . date('Y-m-d H:i:s') . '.' . $extension;
      move_uploaded_file($_FILES['file_variables']['tmp_name'], PATH_DATA . $file_name_destination);
      $inputFileType = PHPExcel_IOFactory::identify(PATH_DATA . $file_name_destination);
      $objReader = PHPExcel_IOFactory::createReader($inputFileType);
      $objPHPExcel = $objReader->load(PATH_DATA . $file_name_destination);
      $objPHPExcel->setActiveSheetIndex('0');
      $objWorksheet = $objPHPExcel->getActiveSheet();
      $nbr_variables = $objWorksheet->getHighestRow()-2;
      $etude = ORM::for_table('etudes')->where('nom_etude',$nom_etude)->where('id_projet',$b_id_projet)->find_one(); 
            $etude->nom_de_fichier=$file;
            $etude->nb_variables=$nbr_variables;
            $etude->save();   
      header("Location: " . "?p=appariement&nom_etude=$nom_etude&pg=1");
    }
  }
}*/
