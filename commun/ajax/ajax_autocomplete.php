...
$b_ajax['results']=ORM::for_table('variables')->raw_query('SELECT * FROM variables WHERE  variables.id_etude="'.$id_etude.'" AND (variable LIKE "%'.$term.'%"  or libelle LIKE "%'.$term.'%" )AND variables.id_var_catalogue is NULL ORDER BY variable ASC')->find_array();
}
