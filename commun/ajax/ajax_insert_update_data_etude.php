...
if (strlen($_GET['file_name']) > 1 && isset($_GET['id_etude']) && isset($_GET['id_projet'])) {
    $separateur = isset($_GET['separateur']) ? urldecode($_GET['separateur']) : ';';
    $etude = ORM::for_table('etudes')->select('id')->select('nom_etude')->select('format')->where('id', $_GET['id_etude'])->find_one();
    $encodage = $etude['encodage'] ? 'UTF-8' : $etude['encodage'];
    ...
    foreach ($p as $val) {
        ...
        if ($val['cle'] == 'CLE')
            $parametre['__cle'][] = $val['variable'];
        if ($val['cle'] == 'ID_PATIENT')
            $parametre['__id'] = $val['variable'];
        if ($val['cle'] == 'VAR')
            $parametre['__var'] = $val['variable'];
        if ($val['cle'] == 'VAL')
            $parametre['__val'] = $val['variable'];
        if ($val['cle'] == 'J0')
            $parametre['__j0'] = $val['variable'];
        if ($val['cle'] == 'IR')
            $parametre['__ir'] = $val['variable'];
    }
    //dump($parametre,true);
    if (!array_key_exists('__cle', $parametre))
        $parametre['__cle'] = 'date_j';
