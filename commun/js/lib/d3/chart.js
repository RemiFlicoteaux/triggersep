...
function dsBarChartPlot(data, target, Title, colorChosen) {
    ...
    x.domain(firstDatasetBarChart.map(function (d) {
        return d.libelle;
    }));
...
function dsBarChart(dataset, target, Title) {
...
console.log(dataChart);
    ...
    bars.append("svg:text")
            ...
            .text(function (d, i) {
                return labellist[i]+' ('+dataChart[i].valeur+')';
            });
...
function printChart(TabData)
{
    ...
    for (var i = 0; i < TabData.length; i++)
    {
        ...
        {
            dsBarChartPlot(TabData[i].dataChart, TabData[i].targetTag, TabData[i].Titre, 0);
        }
        ...
        {
            //alert(i+' : '+TabData[i].dataChart);
            horizontal_chart(TabData[i].dataChart, TabData[i].targetTag);
            //printHorizontalBar(TabData[i].dataChart,TabData[i].dataChart,TabData[i].targetTag,TabData[i].yName);
            //dsBarChart(TabData[i].dataChart, TabData[i].targetTag, TabData[i].Titre);
        }
    }
}
...
// fonction permettant de faire diagramme horizontal
